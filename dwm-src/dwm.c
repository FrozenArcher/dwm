...
enum { SchemeNorm, SchemeSel, SchemeStatus, SchemeTagsSel, SchemeTagsNorm, SchemeTagsDim, SchemeInfoSel, SchemeInfoNorm }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if (m == (statuspinning ? systraytomon(m) : selmon)) { /* status is only drawn on selected monitor */
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
        if (!(occ & 1 << i)) drw_setscheme(drw, scheme[SchemeTagsDim]);
        else drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagsSel : SchemeTagsNorm]);
	...
	if ((w = m->ww - 2*sp - tw - stw - x) > bh) {
		if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
                ...
                cw = TEXTW("A") * minclientlen > TEXTW(c->name) ? TEXTW(" ") * minclientlen : TEXTW(c->name);
				tw = MIN(m->sel == c ? w : mw, cw);
...
getsystraywidth()
{
	...
	return w ? w + systrayspacing + traybarpad : 1;
}
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
			die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
		systray->win = XCreateSimpleWindow(dpy, root, x - sp, m->by + vp, w, bh, 0, 0, scheme[SchemeNorm][ColBg].pixel);
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x, barborder, i->w, i->h);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
